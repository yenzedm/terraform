mod        = "clone"
import_vms = {}
clone_vms = {
  "monitoring-server" = {
    # other
    vm_template_id  = 104
    vm_username     = "user"
    tags            = ["monitoring", "server"]
    target_node     = "proxmox"
    ssh_public_keys = "ssh-rsa  proxmox"
    description     = "monitoring server"
    start_at_boot   = false
    # cpu
    vm_cores = 2
    # mem
    vm_memory = 4096
    # network
    vm_ip_and_mask = "192.168.1.125/24" # "192.168.1.176/24" or "dhcp"
    vm_gateway     = "192.168.1.1"      # if vm_ip_and_mask = "dhcp" set ""
    network_bridge = "vmbr0"
    # disks
    disk_size    = 50
    disk_storage = "local-lvm"
  },
  "monitoring-host1" = {
    # other
    vm_template_id  = 104
    vm_username     = "user"
    tags            = ["monitoring", "host1"]
    target_node     = "proxmox"
    ssh_public_keys = "ssh-rsa  proxmox"
    description     = "monitoring host 1"
    start_at_boot   = false
    # cpu
    vm_cores = 1
    # mem
    vm_memory = 2048
    # network
    vm_ip_and_mask = "192.168.1.126/24" # "192.168.1.176/24" or "dhcp"
    vm_gateway     = "192.168.1.1"      # if vm_ip_and_mask = "dhcp" set ""
    network_bridge = "vmbr0"
    # disks
    disk_size    = 50
    disk_storage = "local-lvm"
  }
}